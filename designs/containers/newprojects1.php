<?php
$data = getAllDataFromTable($select,'projects',$where,$limit);
if(!empty($data))
{
?>
			...
			<table class="table table-bordered table-striped" style="text-align: center;">
				...
				<?php	for($i=0;$i<sizeof($data);$i++)	{	?>
					<tr id="tr-<?php echo $data[$i]['id'];?>">
						...
						<td>
				<?php $blogimgs = array_diff(scandir('uploads/news/'.$data[$i]['id']), array('.','..')); ?>
						...
						<?php for($bim=2;$bim<count($blogimgs)+2;$bim++) { ?>
							<div class="col-md-3">
								<a href="#" data-toggle="modal" data-target="#blogimage-<?php echo $bim; ?>">
									<img class="img-responsive" src="uploads/news/<?php echo $data[$i]['id']; ?>/<?php echo $blogimgs[$bim]; ?>">
								</a>
							</div>
							...
							<div class="modal-dialog">
								<div class="modal-content">
									...
									<div class="modal-body">
										<img src="uploads/news/<?php echo $data[$i]['id']; ?>/<?php echo $blogimgs[$bim]; ?>" class="img-responsive">
									</div>
								</div>
							</div>
							</div>
				...
				<?php } ?>
						...
						<td><a href="project.php?id=<?php echo $data[$i]['id']; ?>" target="_blank"><?php language('preview'); ?></a></td>
						<td><?php echo date('Y-m-d , h:i:sa', $data[$i]['time']); ?></td>
						<td><input type="checkbox" id="<?php echo $data[$i]['id']; ?>" class="checkactive" /></td>
						<td>
							...
							<div id="del-<?php echo $data[$i]['id'];?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<div class="modal-dialog modal-sm">
									<div class="modal-content">
										...
										<div class="modal-body">
										<center>
											<button class="btn btn-danger" onclick="mydelete('<?php echo $data[$i]['id'];?>','projects')" data-dismiss="modal"><?php language('agree'); ?></button>
