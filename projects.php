<?php
	...
	include('develops/projects-front.php');
?>
<!--Author: W3layouts
...
<section class="contact-us" id="contact">
	<h3 class="text-center"><?php language('projects'); ?></h3>
	<p class="text-center">
		<?php if(isset($_SESSION['userid'])) { ?><button type="button" class="btn btn-success" onclick="location.href = 'projects.php'"><?php language('addproject'); ?></button><?php } ?>
	</p>
	<div class="container">	
		<?php if(!empty($projects)) { ?>
		<div class="row">
			<?php for($pr=0;$pr<count($projects);$pr++) { ?>
				<div class="col-lg-4 col-md-4 <?php if($pr%3 == 0) echo 'col-lg-push-8 col-md-push-8'; elseif($pr%3 == 2) echo 'col-lg-pull-8 col-md-pull-8'; ?>">
					<div class="thumbnail" id="product" style="background:rgba(255, 255, 255, 0.5);">
						<div class="caption" style="/*position: absolute;*/ top: 0; bottom: 0; left: 0; right: 0; text-align: center; margin: 55px; padding: 25px; background: rgba(0, 0, 0, 0.5); color: #fff; border-radius: 15px;">
							<h5 style="text-align:center;margin-bottom:25px;"><?php echo $projects[$pr]['name']; ?></h5>
							<br>
							<button type="button" class="btn btn-info" onclick="location.href = 'project.php?id=<?php echo $projects[$pr]['id']; ?>'"><?php language('more'); ?></button>
							<?php if(isset($_SESSION['userid'])) { ?><button type="button" class="btn btn-warning" onclick="location.href = 'editproject.php?id=<?php echo $projects[$pr]['id']; ?>'"><?php language('edit'); ?></button>
							<button type="submit" class="btn btn-danger" onclick="location.href = 'delproject.php?id=<?php echo $projects[$pr]['id']; ?>'"><?php language('delete'); ?></button><?php } ?>
						</div>
					</div>
				</div>
			<?php } ?>
